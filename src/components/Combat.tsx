...
export const Combat: React.FC<CombatProps> = ({ 
  ...
  const [trueFalseAnswers, setTrueFalseAnswers] = useState<number[]>([]);
  const [currentTrueFalseIndex, setCurrentTrueFalseIndex] = useState<number>(0);
  const [trueFalseTimeLeft, setTrueFalseTimeLeft] = useState<number>(10);
  ...
  useEffect(() => {
    ...
    setTrueFalseTimeLeft(10);
    setTimeLeft(question.type === 'true-false-blitz' ? 10 : questionTime);
    setShowResult(false);
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      if (currentQuestion.type === 'true-false-blitz') {
        setTrueFalseTimeLeft(prev => {
          if (prev <= 1) {
            handleAnswer(null);
            return 0;
          }
  ...
  const handleAnswer = (answerIndex: number | null) => {
    ...
    setTimeout(() => {
      ...
      setTimeLeft(newQuestion.type === 'true-false-blitz' ? 10 : questionTime);
      setShowResult(false);
      setLastAnswerCorrect(null);
    }, 2000);
  };
  ...
  const handleTrueFalseAnswer = (answer: number) => {
    if (!currentQuestion?.trueFalseQuestions) return;
    
    const newAnswers = [...trueFalseAnswers, answer];
    setTrueFalseAnswers(newAnswers);
    
    if (currentTrueFalseIndex < currentQuestion.trueFalseQuestions.length - 1) {
  ...
  const renderQuestionInput = () => {
    switch (currentQuestion.type) {
            ...
            <div className="flex flex-wrap gap-2 justify-center">
              {reorderedWords.map((word, index) => (
                <div
                  ...
                  className="bg-purple-600 text-white px-4 py-2 rounded-lg cursor-move hover:bg-purple-500 transition-colors"
                >
                  {word}
                </div>
              ))}
      ...
      case 'true-false-blitz':
        if (!currentQuestion.trueFalseQuestions) return null;
        
        const currentTFQuestion = currentQuestion.trueFalseQuestions[currentTrueFalseIndex];
        
        return (
          <div className="space-y-4">
            <div className="text-center">
              <p className="text-yellow-400 font-bold">
                Question {currentTrueFalseIndex + 1} of {currentQuestion.trueFalseQuestions.length}
              </p>
              <p className="text-red-400 font-bold text-xl">
                Time: {trueFalseTimeLeft}s
              </p>
            </div>
            <div className="bg-black/50 p-4 rounded-lg">
              <p className="text-white text-lg font-semibold text-center mb-4">
                {currentTFQuestion.question}
              </p>
              <div className="grid grid-cols-2 gap-4">
                <button
                  onClick={() => handleTrueFalseAnswer(0)}
                  disabled={isAnswering || showResult}
                  className="bg-green-600 text-white py-3 rounded-lg font-bold hover:bg-green-500 transition-colors"
                >
                  TRUE
                </button>
                <button
                  onClick={() => handleTrueFalseAnswer(1)}
                  disabled={isAnswering || showResult}
                  className="bg-red-600 text-white py-3 rounded-lg font-bold hover:bg-red-500 transition-colors"
                >
                  FALSE
                </button>
              </div>
            ...
            <div className="text-center text-sm text-gray-300">
              Answered: {trueFalseAnswers.length} / {currentQuestion.trueFalseQuestions.length}
            </div>
          </div>
        );
    ...
    <div className="bg-gradient-to-br from-red-900/80 via-purple-900/80 to-black/80 p-6 rounded-xl shadow-2xl backdrop-blur-sm border border-red-500/50">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className={`font-bold text-lg px-3 py-1 rounded-lg ${
              (currentQuestion.type === 'true-false-blitz' ? trueFalseTimeLeft : timeLeft) <= 3 
                ? 'text-red-400 animate-pulse bg-red-900/30' 
                : 'text-yellow-400 bg-yellow-900/30'
            }`}>
              {currentQuestion.type === 'true-false-blitz' ? trueFalseTimeLeft : timeLeft}s
            </span>
          </div>
        </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-purple-400 bg-black/30 px-3 py-1 rounded-lg">
                {currentQuestion.type === 'multiple-choice' ? 'Multiple Choice' : 
                 ...
                 currentQuestion.type === 'true-false-blitz' ? 'True/False Blitz' :
                 'Type Answer'}
              </span>
            </div>
          </div>
      ...
      <div className="text-center mt-4 space-y-2">
        ...
        <p className={`text-sm font-semibold ${
          ...
          ⏰ You have {currentQuestion.type === 'true-false-blitz' ? '10' : questionTime} seconds to answer!
        </p>
        {!hasUsedRevival && (
